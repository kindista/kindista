...
(defun post-push-notification-subscription
 (&aux
    ...
    (client-public-key (getf subscription-keys :p-256-dh))
    (client-auth-secret (getf subscription-keys :auth))
    (status-json (list (cons "subscriptionStatus" "null"))))
; (pprint subscription-keys)
 (pprint client-public-key)
 (pprint client-auth-secret)
  (require-user ()
    (cond
      ((not chrome-p)
