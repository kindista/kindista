...
(defun modify-event (id &key lat long title details local-time address recurring frequency interval days-of-week by-day-or-date weeks-of-month local-end-date)
  (let* ((result (gethash id *db-results*))
    ...
    (flet ((update-when (clause &rest body)
      ...
      (update-unless (and (or details title)
                          (nor (equalp details old-details)
                            (equalp title old-title))
                          )
      ...
      (update-unless (eql local-time (getf data :local-time))
      ...
      (update-unless (and (eq old-recurring recurring)
                          (eql old-frequency frequency)
                          (= old-interval interval)
                          (equal old-days-of-week days-of-week)
                          (eql old-by-day-or-date by-day-or-date)
                          (equal old-weeks-of-month weeks-of-month)
                          (eql old-end-date local-end-date))
                     nil))
